...
<div className='Main-resume'> 
        <img src={logo} className="Main-logo" alt="logo"></img>
                ...
                <div className='Main-resume-intro'>
                    <h2>Опыт работы</h2>
                { person.works.map( el =>  <div>_________________________________________<div><CompanyName company={el.company} />  Начиная с <i>{el.firstDay} по {el.lastDay}</i></div> _________________________________________ <div/> </div> )}
                </div>
                <div className="Main-resume-intro">
                <h2> Навыки</h2>
                    {person.skills.map( el => <div>{el}</div>)}
                </div>
